...
$j(document).ready(function() {
	...
	new Ajax.Updater('logar', 'template/paginas/painel-user.php', {method: 'get', asynchronous:true, evalScripts:true});
	...
	if(url.indexOf("#!")!=-1){
		...
		new Ajax.Updater('container', 'template/paginas/'+pagina[0]+'.php', {method: 'get', asynchronous:true, evalScripts:true});

		mover('#container');

	}else{

		new Ajax.Updater('container', 'template/paginas/home.php?pagina=walkingHome', {method: 'get', asynchronous:true, evalScripts:true});
...
$j(document).ready(function() {
	...
	new Ajax.Updater('rankingHome', 'template/paginas/RankingHome.php', {method: 'get', asynchronous:true, evalScripts:true});
...
function carregando(elemento){
	...
	$j(elemento).html('<div align="center">agara<br /><div class="bubblingG"><span id="bubblingG_1"></span><span id="bubblingG_2"></span><span id="bubblingG_3"></span></div><br /></div>');
...
function loadRank(elemento){
	...
	new Ajax.Updater('rank', 'template/paginas/ranking.php?type='+elemento, {method: 'get', asynchronous:true, evalScripts:true});
...
function reloadCaptcha(){
	...
	$j(".imgCaptcha img").attr("src", "web_config/modules/Captcha/captcha.php?="+d.getTime());
...
function validacion() 
{
var contador=0,limite=1;
for (i=0; i<=document.resgatar.complemento.length; i++)
{
	       if (document.resgatar.complemento[i].checked)
		   {
			   contador=contador+1;
		   }
		   if (contador>limite)
				...
				document.resgatar.complemento[i].checked=false;
				break;
				}
  }
}
